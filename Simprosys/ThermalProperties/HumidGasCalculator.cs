   ...
   public class HumidGasCalculator {
      ...
      public double GetDryBulbFromSatTempHumidityAndPressure(double satTemperature, double humidity, double totalPressure) {
         ...
         do {
            ...
            cpGas = gasProperties.GetMeanSpecificHeatOfDryGas(dryBulbTemperature, satTemperature);
            cpMoisture = moistureProperties.GetMeanSpecificHeatOfVapor(dryBulbTemperature, satTemperature);
         } while(Math.Abs(dryBulbTemperature - dryBulbTemperature_old) / dryBulbTemperature > 1.0e-6 && counter < 200);
      ...
      public double GetHumidEnthalpyFromDryBulbHumidityAndPressure(double temperature, double humidity, double pressure) {
         ...
         double airCp = gasProperties.GetMeanSpecificHeatOfDryGas(273.15, temperature);
         ...
         double liquidMoistureCp = moistureProperties.GetMeanSpecificHeatOfLiquid(273.15, dewPoint);
         double vaporMoistureCp = moistureProperties.GetMeanSpecificHeatOfVapor(dewPoint, temperature);
      ...
      public double GetHumidityFromHumidEnthalpyTemperatureAndPressure(double specificEnthalpy, double temperature, double pressure) {
         ...
         cg = gasProperties.GetMeanSpecificHeatOfDryGas(273.15, temperature);
         ...
         do {
            ...
            specificHeatOfLiquid = moistureProperties.GetMeanSpecificHeatOfLiquid(dewPoint, 273.15);
            specificHeatOfVapor = moistureProperties.GetMeanSpecificHeatOfVapor(dewPoint, temperature);
      ...
      public double GetDryBulbFromHumidEnthalpyHumidityAndPressure(double humidEnthalpy, double humidity, double pressure) {
         ...
         double specificHeatOfLiquid = moistureProperties.GetMeanSpecificHeatOfLiquid(dewPoint, 273.15);
         double evaporationHeat = moistureProperties.GetEvaporationHeat(dewPoint);
         double tg_old;
         double tempValue;
         int counter = 0;
         do {
            ...
            cv = moistureProperties.GetMeanSpecificHeatOfVapor(dewPoint, tg);
      ...
      public double GetHumidHeat(double humidity, double temperature) {
         ...
         return cpGas + moistureProperties.GetSpecificHeatOfVapor(temperature) * humidity;
      }
      ...
      private double CalculatePsychrometricRatio(double temperature, double wetBulbTemperature, double humidity, double pressure) {
         ...
         else if(gas is CompositeSubstance)
         {
            //do nothing so that it takes the default value of 1.0
         }
         else
         {
