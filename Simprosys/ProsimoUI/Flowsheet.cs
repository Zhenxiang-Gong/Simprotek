...
namespace ProsimoUI {
   ...
   public class Flowsheet : Panel, ISerializable {
      ...
      public virtual void SetObjectData() {
         ...
         {
            RecallInitializationLocal();
         }
      }

      private void RecallInitializationLocal()
      {
      ...
      public void RecallInitialization(int majorVersion, int minorVersion)
      {
         if(majorVersion <= 2)
         {
            //Initialize the newly added material stream in BagFilter and ElectrostaticPrecipitator in Simprosys 3.0
            this.evapAndDryingSystem.RecallInitialization();
            //initialize the UI connection between newly added material stream and the unit operation
            this.unitOpManager.RecallInitialization();
         }
      }
