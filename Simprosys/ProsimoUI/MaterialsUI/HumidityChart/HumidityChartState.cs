...
namespace ProsimoUI.HumidityChart
{
   ...
   public class HumidityChartState
   {
      ...
      public override string ToString()
      {
         ...
         {
            str = "x = " + this.variables.X.ToString(this.flowsheet.ApplicationPrefs.NumericFormatString) +
               ", y = " + this.variables.Y.ToString(this.flowsheet.ApplicationPrefs.NumericFormatString);
         }
         else if (this.type.Equals(HCStateType.ProcessInput))
         {
            str = "IN: x = " + this.variables.X.ToString(this.flowsheet.ApplicationPrefs.NumericFormatString) +
               ", y = " + this.variables.Y.ToString(this.flowsheet.ApplicationPrefs.NumericFormatString);
         }
         else if (this.type.Equals(HCStateType.ProcessOutput))
         {
            str = "OUT: x = " + this.variables.X.ToString(this.flowsheet.ApplicationPrefs.NumericFormatString) +
               ", y = " + this.variables.Y.ToString(this.flowsheet.ApplicationPrefs.NumericFormatString);
         }
         return str;         
      }
