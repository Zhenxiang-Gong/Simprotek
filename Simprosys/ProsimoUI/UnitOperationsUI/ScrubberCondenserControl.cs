...
namespace ProsimoUI.UnitOperationsUI {
   ...
   public class ScrubberCondenserControl : UnitOpControl {
      ...
      public static PointOrientation GAS_OUTLET_ORIENTATION = PointOrientation.W;
      public static PointOrientation LIQUID_OUTLET_ORIENTATION = PointOrientation.S;
      public static PointOrientation WATER_INLET_ORIENTATION = PointOrientation.E;
      public static PointOrientation WATER_OUTLET_ORIENTATION = PointOrientation.E;
      
      private static IDictionary<SolveState, Image> backgroundImageTable;

      internal protected override string SolvableTypeName
      {
         get { return "Scrubber Condenser"; }
      }
      ...
      static ScrubberCondenserControl() {
         backgroundImageTable = new Dictionary<SolveState, Image>();
         backgroundImageTable.Add(SolveState.NotSolved, UI.IMAGES.SCRUBBERCONDENSER_CTRL_NOT_SOLVED_IMG);
         backgroundImageTable.Add(SolveState.SolvedWithWarning, UI.IMAGES.SCRUBBERCONDENSER_CTRL_SOLVED_WITH_WARNING_IMG);
         backgroundImageTable.Add(SolveState.Solved, UI.IMAGES.SCRUBBERCONDENSER_CTRL_SOLVED_IMG);
         backgroundImageTable.Add(SolveState.SolveFailed, UI.IMAGES.SCRUBBERCONDENSER_CTRL_SOLVE_FAILED_IMG);
      }
      ...
      public bool HitTestWaterIn(Point p) {
         bool hit = false;
         Point slot = this.GetWaterInSlotPoint();
         GraphicsPath gp = new GraphicsPath();
      ...
      public bool HitTestWaterOut(Point p) {
         bool hit = false;
         Point slot = this.GetWaterOutSlotPoint();
         GraphicsPath gp = new GraphicsPath();
         gp.AddLine(this.Width, slot.Y - UI.SLOT_DELTA, this.Width, slot.Y + UI.SLOT_DELTA);
         hit = gp.IsOutlineVisible(p, penBlack20, g);
      ...
      protected override void MouseDownHandler(Point p) {
         if (this.flowsheet.Activity == FlowsheetActivity.AddingConnStepOne) {
            int idx = -1;
            if (HitTestGasIn(p) || HitTestGasOut(p) || HitTestLiquidOut(p) || HitTestWaterIn(p) || HitTestWaterOut(p)) {
               if (HitTestGasIn(p))
               ...
               else if (HitTestWaterIn(p))
                  idx = ScrubberCondenser.WATER_INLET_INDEX;
               else if (HitTestWaterOut(p))
                  idx = ScrubberCondenser.WATER_OUTLET_INDEX;
      ...
      protected override void DrawSlots() {
         ...
         this.DrawSlot(this, this.GetGasOutSlotPoint(), SlotPosition.Left, SlotDirection.Out);
         this.DrawSlot(this, this.GetLiquidOutSlotPoint(), SlotPosition.Down, SlotDirection.Out);
         this.DrawSlot(this, this.GetWaterInSlotPoint(), SlotPosition.Right, SlotDirection.In);
         this.DrawSlot(this, this.GetWaterOutSlotPoint(), SlotPosition.Right, SlotDirection.Out);
      }
      ...
      public Point GetWaterInConnectionPoint() {
         // this point is referenced to the flowsheet
         // middle of the left side
         Point p1 = this.GetWaterInSlotPoint();
         Point p2 = this.Location;
         return new Point(p1.X + p2.X, p1.Y + p2.Y);
      }

      public Point GetWaterInSlotPoint() {
         // this point is referenced to this control
         // middle of the left side
         Point p = new Point(0, 0);
         p.X = this.Width;
      ...
      public Point GetWaterOutConnectionPoint() {
         // this point is referenced to the flowsheet
         // middle of the up side
         Point p1 = this.GetWaterOutSlotPoint();
         Point p2 = this.Location;
         return new Point(p1.X + p2.X, p1.Y + p2.Y);
      }

      public Point GetWaterOutSlotPoint() {
         // this point is referenced to this control
         // middle of the up side
         Point p = new Point(0, 0);
         p.X = this.Width;
...
//   //sb.Append("Scrubber Condenser: ").Append(this.ScrubberCondenser.Name).Append("\r\n");
//   //sb.Append(UI.UNDERLINE).Append("\r\n");

//   //sb.Append(ToPrintVarList());

//   //return sb.ToString();
//   return ToPrintVarList("Scrubber Condenser: ");
//}
