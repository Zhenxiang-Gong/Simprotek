...
namespace Prosimo.Materials {
   ...
   public class CompositeSubstance : Substance {
      ...
      public override double MolarWeight
      {
         get { return molarWeight; }
      }

      private void CalculateMolarWeight()
      {
         double total = 0.0;
         foreach(MaterialComponent sc in components)
         {
            total += sc.MoleFraction.Value * sc.Substance.MolarWeight;
         }
      ...
      public void Normalize() {
         ...
         if (total > 1.0e-8) {
            foreach (MaterialComponent sc in components) {
               sc.MoleFraction.Value = sc.MassFraction.Value / sc.Substance.MolarWeight/total;
            }
         }
      }
