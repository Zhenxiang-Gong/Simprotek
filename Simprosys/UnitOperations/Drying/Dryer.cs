...
namespace Prosimo.UnitOperations.Drying {
   ...
   public abstract class Dryer : UnitOperation {
      ...
      //for accurate calculation of the specific heat consumption
      private ProcessVarDouble originalGasTemperature;
      ...
      public ProcessVarDouble OriginalGasTemperature {
         get { return originalGasTemperature; }
      }
      ...
      protected Dryer(string name, UnitOperationSystem uoSys)
         ...
         specificHeatConsumption = new ProcessVarDouble(StringConstants.SPECIFIC_HEAT_CONSUMPTION, PhysicalQuantity.SpecificEnergy, VarState.AlwaysCalculated, this);
         ...
         originalGasTemperature = new ProcessVarDouble(StringConstants.INITIAL_GAS_TEMPERATURE, PhysicalQuantity.Temperature, 293.15, VarState.Specified, this);
      ...
      private void InitializeVarListAndRegisterVars() {
         ...
         AddVarOnListAndRegisterInSystem(originalGasTemperature);
      ...
      {
         return GetHumidGasCalculator(gasInlet);
      }
      ...
      private void Solve() {
         ...
         double csInlet = materialInlet.GetCpOfAbsoluteDryMaterial();
         double csOutlet = materialOutlet.GetCpOfAbsoluteDryMaterial();
         double cs = MathUtility.Average(csInlet, csOutlet);
         ...
         if (solveState == SolveState.Solved) {
            ...
            //   }
            //}
            if (originalGasTemperature.HasValue) {
               double q0 = wg * cg1 * (tg1 - originalGasTemperature.Value);
      ...
      private void CalculateMaterialOutletComponents() {
         ...
         if (materialOutMassFlow != Constants.NO_VALUE && fractionOfMaterialLostToGasOutlet.HasValue) {
            ...
            double materialMoistureContentWetBasis = materialMoistureContent / (1.0 + materialMoistureContent);
            ...
            if (dgc.NumberOfPhases <= 1) {
               ...
               mc.SetMassFractionValue(1.0 - materialMoistureContentWetBasis);
               solidCompList.Add(mc);
               mc = new MaterialComponent(dgc.Moisture.Substance);
               mc.SetMassFractionValue(materialMoistureContentWetBasis);
               solidCompList.Add(mc);
               ...
               sp[0].SetMassFractionValue(1.0 - materialMoistureContentWetBasis);
               sp[1].SetMassFractionValue(materialMoistureContentWetBasis);
               sp.MassFraction = massFlowRateOfMaterialLost / gasOutMassFlow;
            }
         }
      }
      ...
      public override void SetObjectData() {
         ...
         specificHeatConsumption.Type = PhysicalQuantity.SpecificEnergy;
         ...
         if (persistedClassVersion == 1) {
            this.originalGasTemperature = new ProcessVarDouble(StringConstants.INITIAL_GAS_TEMPERATURE, PhysicalQuantity.Temperature, 293.15, VarState.Specified, this);
            varList.Insert(6, originalGasTemperature);
            unitOpSystem.RegisterProcessVar(this.originalGasTemperature);
         }
         else if (persistedClassVersion == 2) {
            this.originalGasTemperature = RecallStorableObject("OriginalGasTemperature", typeof(ProcessVarDouble)) as ProcessVarDouble;
         }
      }
      ...
      public override void GetObjectData(SerializationInfo info, StreamingContext context) {
         ...
         //version 2 addition begin
         info.AddValue("OriginalGasTemperature", this.originalGasTemperature, typeof(ProcessVarDouble));
