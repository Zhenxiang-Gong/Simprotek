...
namespace Prosimo.UnitOperations {
   ...
   public class EvaporationAndDryingSystem : UnitOperationSystem {
      private const int CLASS_PERSISTENCE_VERSION = 3;
      ...
      private IDictionary<Substance, HumidGasCalculator> humidGasCalculatorTable = new Dictionary<Substance, HumidGasCalculator>();
      ...
      public EvaporationAndDryingSystem(string name, DryingMaterial material, DryingGas gas, FossilFuel fuel)
         : base(name) {
         this.dryingMaterial = material;
         this.dryingGas = gas;
         this.dryingFuel = fuel;
      ...
      public HumidGasCalculator GetHumidGasCalculator(Substance s)  {
         if(humidGasCalculatorTable.ContainsKey(s))
         {
            return humidGasCalculatorTable[s];
         }
         else
         {
            HumidGasCalculator humidGasCalculator = new HumidGasCalculator(s, dryingMaterial.Moisture);
            humidGasCalculatorTable.Add(s, humidGasCalculator);
      ...
      private void InitilizeMaps() {
         ...
         solvableNameStringMap.Add(typeof(DetailedFuelStream), "Fuel ");
         //solvableNameStringMap.Add(typeof(GenericFuelStream), "Fuel ");
         ...
         solvableNameStringMap.Add(typeof(Burner), "Burner ");
      }
      ...
      public IList<Type> GetSolvableTypeList() {
         ...
         if (dryingMaterial.Moisture.IsWater) {
            ...
            solvableTypeList.Add(typeof(DetailedFuelStream));
            //solvableTypeList.Add(typeof(GenericFuelStream));
            solvableTypeList.Add(typeof(SolidDryer));
            solvableTypeList.Add(typeof(LiquidDryer));
            solvableTypeList.Add(typeof(Fan));
            solvableTypeList.Add(typeof(Burner)); 
            ...
            solvableTypeList.Add(typeof(SolidDryingMaterialStream));
            solvableTypeList.Add(typeof(LiquidDryingMaterialStream));
            //only make the fule stream and burner available when drying gas is air
            if (dryingGas.Substance.IsAir)
            {
               solvableTypeList.Add(typeof(DetailedFuelStream));
            }
      ...
      private void RecallInitilization(DryingMaterial recalledDryingMaterial, DryingGas recalledDryingGas, FossilFuel recalledDryingFuel) {
         ...
         if (dryingMaterialInCatalog == null) {
            ...
            dryingMaterial = dryingMaterialInCatalog;
         }
         else {
            dryingMaterial = recalledDryingMaterial;
         ...
         FossilFuelCatalog fuelCatalog = FossilFuelCatalog.Instance;
         if (recalledDryingFuel == null)
         {
            recalledDryingFuel = (FossilFuel)fuelCatalog.GetFossilFuelList()[0];
         }
         FossilFuel fossilFuelInCatalog = fuelCatalog.GetFossilFuel(recalledDryingFuel.Name);
         if (fossilFuelInCatalog == null)
         {
            dryingFuel = recalledDryingFuel;
            fuelCatalog.AddFossilFule(recalledDryingFuel);
         }
         else if (fossilFuelInCatalog.Equals(recalledDryingFuel))
         {
            dryingFuel = fossilFuelInCatalog;
         }
         else
         {
            dryingFuel = recalledDryingFuel;
            string fuelName = dryingFuel.Name + " For " + this.name;
            dryingFuel.Name = fuelName;
            fossilFuelInCatalog = fuelCatalog.GetFossilFuel(fuelName);
            if (fossilFuelInCatalog != null && !fossilFuelInCatalog.Equals(recalledDryingFuel))
            {
               fuelName = fuelCatalog.GetUniqueFossilFuleName(fuelName);
            }
            dryingFuel.Name = fuelName;
            dryingFuel.IsUserDefined = true;
            fuelCatalog.AddFossilFule(recalledDryingFuel);
         }
      ...
      /// <summary>
      /// Initialize the newly added material stream in BagFilter and ElectrostaticPrecipitator in Simprosys 3.0
      /// </summary>
      public void RecallInitialization()
      {
         foreach(UnitOperation unitOp in GetUnitOpList())
         {
            if(unitOp is BagFilter || unitOp is ElectrostaticPrecipitator)
            {
               ProcessStreamBase ps = MakeDryingMaterialStream(MaterialStateType.Solid);
               OnStreamAdded(ps);
               unitOp.DoAttach(ps, BagFilter.PARTICLE_OUTLET_INDEX);
               unitOp.HasBeenModified(true);
            }
         }
      }
      ...
      private UnitOperation CreateUnitOperation(Type unitOpType) {
         ...
         else if(unitOpType == typeof(Burner))
         {
            unitOp = MakeBurner(creationType);
         }
      ...
      private ProcessStreamBase MakeStream(Type streamType) {
         ...
         else if(streamType == typeof(DetailedFuelStream))
         {
            ps = MakeDetailedFuelStream();
         }
         //else {
     ...
     private DryingMaterialStream MakeDryingMaterialStream(MaterialStateType materialType) {
      ...
      private DetailedFuelStream MakeDetailedFuelStream()
      {
         string name = GetUniqueSolvableName(typeof(DetailedFuelStream));

         //MaterialComponents mComponents = new MaterialComponents(fossilFuel.ComponentList);
         DetailedFuelStream processStream = new DetailedFuelStream(name, dryingFuel, this);
         processStream.DryingFuel = dryingFuel;
         streamList.Add(processStream);
         return processStream;
      }

      private Burner MakeBurner(UnitOpCreationType creationType)
      {
         string name = GetUniqueSolvableName(typeof(Burner));
         Burner burner = new Burner(name, this);
         unitOpList.Add(burner);
         ...
         {
            ps = MakeDetailedFuelStream();
            burner.DoAttach(ps, Burner.FUEL_INLET_INDEX);
            ps = MakeDryingGasStream();
            burner.DoAttach(ps, Burner.AIR_INLET_INDEX);
            ps = MakeDryingGasStream();
            burner.DoAttach(ps, Burner.FLUE_GAS_OUTLET_INDEX);
         }
      ...
      private Heater MakeHeater(Type streamType, UnitOpCreationType creationType) {
         ...
         Heater unitOp = new Heater(name, this);
         //heaterList.Add(heater);
         unitOpList.Add(unitOp);

         ProcessStreamBase ps;
         if (creationType == UnitOpCreationType.WithInputAndOutput) {
            ps = MakeStream(streamType);
            unitOp.DoAttach(ps, TwoStreamUnitOperation.INLET_INDEX);

            ps = MakeStream(streamType);
            unitOp.DoAttach(ps, TwoStreamUnitOperation.OUTLET_INDEX);
         }
         else if (creationType == UnitOpCreationType.WithInputOnly) {
            ps = MakeStream(streamType);
            unitOp.DoAttach(ps, TwoStreamUnitOperation.INLET_INDEX);
      ...
      private Cooler MakeCooler(Type streamType, UnitOpCreationType creationType) {
         ...
         Cooler unitOp = new Cooler(name, this);
      ...
      private BagFilter MakeBagFilter(UnitOpCreationType creationType) {
         ...
         BagFilter unitOp = new BagFilter(name, this);
         unitOpList.Add(unitOp);
      ...
      private AirFilter MakeAirFilter(UnitOpCreationType creationType) {
         ...
         AirFilter unitOp = new AirFilter(name, this);
         unitOpList.Add(unitOp);
      ...
      private ElectrostaticPrecipitator MakeElectrostaticPrecipitator(UnitOpCreationType creationType) {
         ...
         ElectrostaticPrecipitator unitOp = new ElectrostaticPrecipitator(name, this);
         unitOpList.Add(unitOp);
      ...
      public override void SetObjectData() {
         ...
         if (persistedClassVersion >= 3)
         {
            dryingFuel = RecallStorableObject("DryingFuel", typeof(FossilFuel)) as FossilFuel;
         }
         RecallInitilization(recalledDryingMaterial, recalledDryingGas, dryingFuel);
      }
      ...
      public override void GetObjectData(SerializationInfo info, StreamingContext context) {
         ...
         //version 3.0
         info.AddValue("DryingFuel", this.dryingFuel, typeof(FossilFuel));
      }
      #endregion Persistence
   }
}

//private DryingGasStream MakeAirStream()
//{
//   ArrayList compList = new ArrayList();
//   SubstanceCatalog catalog = SubstanceCatalog.GetInstance();
//   Substance air = catalog.GetSubstance(Substance.AIR);
//   MaterialComponent mc = new MaterialComponent(air);
//   compList.Add(mc);
//   mc = new MaterialComponent(dryingMaterial.Moisture);
//   compList.Add(mc);
//   MaterialComponents dryingGasComponents = new DryingGasComponents(compList);
...
//   ArrayList list = GetSolvableList(typeof(DryingGasStream));
//   string streamName = GetUniqueName(list, "Air ");

//   DryingGasStream airStream = new DryingGasStream(streamName, dryingGasComponents, this);
//   streamList.Add(airStream);
//   return airStream;
//}
