   ...
   public class DryingMaterialStream : DryingStream {
      ...
      internal double GetBoilingPoint(double pressureValue, double massConcentrationValue) {
         ...
         if (massConcentrationValue > 1.0e-4) {
            DryingMaterial dm = DryingMaterial;
      ...
      internal double GetCpOfAbsoluteDryMaterial() {
         ...
         double tempValue = temperature.HasValue ? temperature.Value : 298.15;
         return GetCpOfAbsoluteDryMaterial(tempValue);
      }

      internal double GetCpOfAbsoluteDryMaterial(double tempValue) {
         double cs = Constants.NO_VALUE;

         if (DryingMaterial.MaterialType == MaterialType.GenericMaterial)
         {
            cs = DryingMaterial.SpecificHeatOfAbsoluteDryMaterial;
         }
         else if (DryingMaterial.MaterialType == MaterialType.GenericFood)
         {
            cs = GenericFoodPropCalculator.GetAbsoluteDryCp((CompositeSubstance)DryingMaterial.AbsoluteDryMaterial, tempValue);
         }
         else {
      ...
      public override void Execute(bool propagate) {
         ...
         else if (specificEnthalpy.HasValue && DryingMaterial.MaterialType == MaterialType.GenericMaterial && moistureContentDryBase.HasValue && !specificHeat.IsSpecifiedAndHasValue)
         {
         ...
         if (materialStateType == MaterialStateType.Solid && (DryingMaterial.MaterialType == MaterialType.GenericMaterial && specificHeat.HasValue || 
            DryingMaterial.MaterialType == MaterialType.GenericFood))
         {
            if (temperature.HasValue && moistureContentDryBase.HasValue) {
               ...
               double tValue = CalculateTemperatureFromEnthalpyForSolid(specificEnthalpy.Value, moistureContentWetBase.Value, out cs);
      ...
      private double CalculateTemperatureFromEnthalpyForSolid(double hValue, double moistureContentValue, out double cs) {
         ...
         do {
            counter++;
            tValueOld = tValue;
            cs = GetCpOfAbsoluteDryMaterial(tValue);
