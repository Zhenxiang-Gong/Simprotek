...
namespace Prosimo.UnitOperations.GasSolidSeparation {
   ...
   public class ScrubberCondenser : UnitOperation, IGasSolidSeparator {
      ...
      public static int WATER_INLET_INDEX = 3;
      public static int WATER_OUTLET_INDEX = 4;
      ...
      protected ProcessStreamBase waterInlet;
      protected ProcessStreamBase waterOutlet;
      //private bool includeCoolingStreams = false;
      ...
      public ProcessStreamBase WaterInlet {
         get { return waterInlet; }
      }

      //implement interface IGasSolidSeparator
      public ProcessStreamBase WaterOutlet {
         get { return waterOutlet; }
      }
      ...
      public override bool CanAttach(int streamIndex) {
         ...
         else if (streamIndex == WATER_INLET_INDEX && waterInlet == null) {
            retValue = true;
         }
         else if (streamIndex == WATER_OUTLET_INDEX && waterOutlet == null) {
            retValue = true;
         }
      ...
      public override bool CanAttachStream(ProcessStreamBase ps, int streamIndex) {
         if (((streamIndex == GAS_INLET_INDEX || streamIndex == WATER_INLET_INDEX) && ps.DownStreamOwner != null)
            || ((streamIndex == GAS_OUTLET_INDEX || streamIndex == LIQUID_OUTLET_INDEX || streamIndex == WATER_OUTLET_INDEX) && ps.UpStreamOwner != null)) {
            return false;
         }
         ...
         if (ps is DryingGasStream) {
            ...
            if (streamIndex == LIQUID_OUTLET_INDEX) {
               canAttach = true;
            }
            else if (((EvaporationAndDryingSystem)UnitOpSystem).DryingMaterial.Moisture.IsWater){
               if (streamIndex == WATER_INLET_INDEX && waterInlet == null) {
                  canAttach = true;
               }
               else if (streamIndex == WATER_OUTLET_INDEX && waterOutlet == null) {
                  canAttach = true;
      ...
      internal override bool DoAttach(ProcessStreamBase ps, int streamIndex) {
         ...
         else if (streamIndex == WATER_INLET_INDEX) {
            waterInlet = ps;
            ps.DownStreamOwner = this;
            inletStreams.Add(ps);
         }
         else if (streamIndex == WATER_OUTLET_INDEX) {
            waterOutlet = ps;
            ps.UpStreamOwner = this;
            outletStreams.Add(ps);
         }
      ...
      internal override bool DoDetach(ProcessStreamBase ps) {
         ...
         else if (ps == waterInlet) {
            waterInlet = null;
            ps.DownStreamOwner = null;
            inletStreams.Remove(ps);
         }
         else if (ps == waterOutlet) {
            waterOutlet = null;
            ps.UpStreamOwner = null;
            outletStreams.Remove(ps);
         }
      ...
      public override void Execute(bool propagate) {
         ...
         if (waterInlet != null && waterOutlet != null) {
            BalanceProcessStreamFlow(waterInlet, waterInlet);
         }
      ...
      private void Solve() {
         ...
         if (dmsOutlet.Temperature.HasValue && dgsInlet.SpecificEnthalpyDryBase.HasValue && coolingDuty.HasValue
            ...
            && dgsInlet.MassFlowRateDryBase.HasValue && dgsOutlet.SpecificEnthalpyDryBase.HasValue
            && inletDustMoistureFraction != Constants.NO_VALUE && materialFromGas != Constants.NO_VALUE) {
            ...
            if (waterInlet != null && waterOutlet != null) {
               ...
               else if (waterInlet.SpecificEnthalpy.HasValue && waterOutlet.SpecificEnthalpy.HasValue) {
                  double waterMassFlowRate = totalEnthapyLoss/(waterOutlet.SpecificEnthalpy.Value - waterInlet.SpecificEnthalpy.Value);
                  Calculate(waterInlet.MassFlowRate, waterMassFlowRate);
                  Calculate(waterOutlet.MassFlowRate, waterMassFlowRate);
               }
            }
         }
         ...
         //   && dmsOutlet.MoistureContentWetBase.HasValue) {
            ...
            && ((waterInlet == null || waterOutlet == null) ||
                (waterInlet != null && waterOutlet != null) &&
                  (waterInlet.MassFlowRate.HasValue && waterOutlet.SpecificEnthalpy.HasValue
                  || waterOutlet.MassFlowRate.HasValue && waterInlet.SpecificEnthalpy.HasValue))) {
            solveState = SolveState.Solved;
         }
      }
      ...
      public override void SetObjectData() {
         ...
         if (persistedClassVersion >= 2) {
            this.waterInlet = info.GetValue("WaterInlet", typeof(ProcessStreamBase)) as ProcessStreamBase;
            this.waterOutlet = info.GetValue("WaterOutlet", typeof(ProcessStreamBase)) as ProcessStreamBase;
         }
      }
      ...
      public override void GetObjectData(SerializationInfo info, StreamingContext context) {
         ...
         //version 2 addition
         info.AddValue("WaterInlet", this.waterInlet, typeof(ProcessStreamBase));
         info.AddValue("WaterOutlet", this.waterOutlet, typeof(ProcessStreamBase));
         //version 2 addition
