...
namespace Prosimo.UnitOperations {
   ...
   public abstract class UnitOperation : Solvable {
      ...
      public virtual bool CanAttach(int streamIndex) {
         return false;
      }
      ...
      protected void BalanceDryingGasStreamFlow(DryingGasStream inlet, DryingGasStream outlet) {
         //at first, make sure the dry medium is the same since there is a possibility that inelt and outlet dry medium is different
         //due to the possibility of combustion
         if (inlet.GasComponents.DryMedium != outlet.GasComponents.DryMedium)
         {
            outlet.GasComponents.DryMedium = inlet.GasComponents.DryMedium;
         }
      ...
      protected void AdjustVarsStates(ProcessStreamBase inlet, ProcessStreamBase outlet) {
         if (inlet.MassFlowRate.IsSpecifiedAndHasValue)
         //if (dsInlet.MassFlowRate.HasValue) 
         {
            outlet.MassFlowRate.State = VarState.Calculated;
            outlet.VolumeFlowRate.State = VarState.Calculated;
            outlet.HasVarStateChanged = true;
         }
         else if (inlet.VolumeFlowRate.IsSpecifiedAndHasValue)
         //else if (dsInlet.VolumeFlowRate.HasValue)
         {
            outlet.MassFlowRate.State = VarState.Calculated;
            outlet.VolumeFlowRate.State = VarState.Calculated;
         ...
         else if (outlet.MassFlowRate.IsSpecifiedAndHasValue)
         //else if (dsOutlet.MassFlowRate.HasValue)
         {
            inlet.MassFlowRate.State = VarState.Calculated;
            inlet.VolumeFlowRate.State = VarState.Calculated;
            inlet.HasVarStateChanged = true;
         }
         else if (outlet.VolumeFlowRate.IsSpecifiedAndHasValue)
         //else if (dsOutlet.VolumeFlowRate.HasValue)
         {
            inlet.MassFlowRate.State = VarState.Calculated;
            inlet.VolumeFlowRate.State = VarState.Calculated;
