   ...
   public class HeatExchanger : UnitOperation {
      ...
      public override ErrorMessage SpecifyCalculationType(UnitOpCalculationType aValue) {
         ...
         if (aValue == UnitOpCalculationType.Rating) {
            if (!InletsArePureLiquids()) {
               retMsg = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, "Heat exchanger rating is not supported yet when material stream is not pure water");
               return retMsg;
            }
         }
      ...
      private bool InletsArePureLiquids() {
         ...
         if (dms != null && dms.MassConcentration.HasValue && dms.MassConcentration.Value > 1.0e-6) {
            retValue = true;
         }
         else {
      ...
      public override void Execute(bool propagate) {
         if (!InletsArePureLiquids() && calculationType == UnitOpCalculationType.Rating) {
