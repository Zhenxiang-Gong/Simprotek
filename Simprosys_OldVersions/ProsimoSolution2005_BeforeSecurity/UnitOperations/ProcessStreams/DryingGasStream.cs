...
namespace Prosimo.UnitOperations.ProcessStreams {
   ...
   public class DryingGasStream : DryingStream {
      ...
      //humidity for drying gas is actually dry base moisture content
      ...
      private void InitializeVarListAndRegisterVars() {
         ...
         //The following vars should not included in drying gas stream and should not appear on the 
      ...
      protected override ErrorMessage CheckSpecifiedValueRange(ProcessVarDouble pv, double aValue) {
         ...
         if (pv == temperature) {
            if (aValue != Constants.NO_VALUE && wetBulbTemperature.IsSpecifiedAndHasValue && aValue < wetBulbTemperature.Value) {
            ...
            else if (aValue != Constants.NO_VALUE && dewPoint.IsSpecifiedAndHasValue && aValue < dewPoint.Value) {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " can not be less than " + StringConstants.GetTypeName(StringConstants.DEW_POINT_TEMPERATURE));
            }
         }
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.WET_BULB_TEMPERATURE)) 
         else if (pv == wetBulbTemperature) {
            if (aValue != Constants.NO_VALUE && temperature.IsSpecifiedAndHasValue && aValue > temperature.Value) {
               ...
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " can not be less than than " + StringConstants.GetTypeName(StringConstants.DEW_POINT_TEMPERATURE));
            }
         }
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.DEW_POINT))
            ...
            else if (aValue != Constants.NO_VALUE && wetBulbTemperature.IsSpecifiedAndHasValue && aValue > wetBulbTemperature.Value) {
      ...
      public override void SetObjectData() {
         base.SetObjectData();
         int persistedClassVersion = (int)info.GetValue("ClassPersistenceVersionDryingGasStream", typeof(int));
