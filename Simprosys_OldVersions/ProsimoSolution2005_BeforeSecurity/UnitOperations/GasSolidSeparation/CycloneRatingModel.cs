...
//         }
      ...
      internal ErrorMessage CheckSpecifiedValueRange(ProcessVarDouble pv, double aValue) 
      {
         ...
         //if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.INLET_WIDTH))
         if (pv == inletWidth)
         {
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.INLET_HEIGHT))
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.INLET_HEIGHT_TO_WIDTH_RATIO)) 
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.OUTLET_INNER_DIAMETER)) 
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.OUTLET_WALL_THICKNESS)) 
         ...
         else if (pv == diplegDiameter)
         {
            ...
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " can not be greater than " + cycloneDiameter.VarTypeName);
            }
         ...
         {
            if (aValue != Constants.NO_VALUE && (aValue <= 0.0 || aValue >= Math.PI/2)) 
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " of the cyclone must be in the range of 0 to 90 degrees");
            }
         }
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.CUT_PARTICLE_DIAMETER)) 
         else if (pv == cutParticleDiameter) 
         {
            if (!(aValue == Constants.NO_VALUE || aValue > 0.0 && aValue < 0.001)) 
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " must be in the range of 0 to 1000 µm");
            }
         }
            //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.PARTICLE_DENSITY)) 
         else if (pv == ParticleDensity) 
         {
            ...
            //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.PARTICLE_BULK_DENSITY)) 
