			...
			if (_windows.Contains(w))
			{
				...
				if (_windows.Count > 1)
				{
                  ...
                  Decimal diff = w.ZoneArea - newSpace;					
                     ...
                     // DMD Jan-06-04 - replaced "diff > 0m" with "extra > 0m"
                     // to get around endless loop created by quickly opening
                     // and closing tabbed windows via the keyboard. This
                     // caused diff to become so small that dividing it by any
                     // decimal returned an "extra" value of 0.
                     while(extra > 0m)
							{
								foreach(Window target in _windows)
								{
