...
namespace Prosimo.UnitOperations.ProcessStreams {
   ...
   /// </summary>
   [Serializable]
   public class LiquidDryingMaterialStream : DryingMaterialStream {
      private const int CLASS_PERSISTENCE_VERSION = 1;

      public LiquidDryingMaterialStream(string name, MaterialComponents mComponents, UnitOperationSystem uoSys)
         : base(name, mComponents, MaterialStateType.Liquid, uoSys) {
      }

      #region Persistence
      protected LiquidDryingMaterialStream(SerializationInfo info, StreamingContext context)
         : base(info, context) {
      }
