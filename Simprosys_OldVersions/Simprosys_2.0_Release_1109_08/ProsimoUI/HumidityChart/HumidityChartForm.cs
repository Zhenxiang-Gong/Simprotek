...
namespace ProsimoUI.HumidityChart
{
   ...
   public class HumidityChartForm : System.Windows.Forms.Form, IPlot
   {
      ...
      void HumidityChartForm_ResizeEnd(object sender, EventArgs e)
      {
      ...
      {
         ErrorMessage error = this.psychrometricChartModel.SpecifyHCType(HCType.GasState);
         if (error != null)
            UI.ShowError(error);
      }
      ...
      {
         ErrorMessage error = this.psychrometricChartModel.SpecifyHCType(HCType.IsenthalpicProcess);
         if (error != null)
            UI.ShowError(error);
      }
