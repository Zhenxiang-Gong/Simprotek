...
namespace Prosimo.UnitSystems
{
   ...
   public class PressureUnit {
      ...
      public static double ConvertToSIValue(PressureUnitType unitType, double toBeConvertedValue) {
         double convertionCoeff = (double) coeffTable[unitType];
         return convertionCoeff * toBeConvertedValue;
      }
      
      public static double ConvertFromSIValue(PressureUnitType unitType, double pascalValue) {
         double convertionCoeff = (double) coeffTable[unitType];
         return pascalValue/convertionCoeff;
      }
      
      public static string GetUnitAsString(PressureUnitType unitType) {
         return unitStringTable[unitType] as String;
      }
   
      public static PressureUnitType GetUnitAsEnum(string unitString) {
         IDictionaryEnumerator myEnumerator = unitStringTable.GetEnumerator();
         String name;
         PressureUnitType type = PressureUnitType.NewtonPerSquareMeter;
         while (myEnumerator.MoveNext()) {
            name = myEnumerator.Value as String;
            if (name.Equals(unitString)) {
               type = (PressureUnitType) myEnumerator.Key;
               break;
            }
         }
         return type;
