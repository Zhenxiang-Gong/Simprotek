...
namespace Prosimo.UnitOperations.GasSolidSeparation {
   ...
   public class Cyclone : UnitOperation, IGasSolidSeparator {
      ...
      protected override void EnableRatingModel() {
         //in cyclone rating collection efficiency cannot be specified
      ...
      internal override ErrorMessage CheckSpecifiedValueInContext(ProcessVarDouble pv, double aValue) {
         ...
         if (pv == gasInlet.Pressure && gasOutlet.Pressure.HasValue && aValue < gasOutlet.Pressure.Value) {
            retValue = CreateSimpleGenericInappropriateSpecifiedValueErrorMessage(pv.VarTypeName + " of the cyclone gas inlet must be greater than that of the outlet.");
         }
         else if (pv == gasOutlet.Pressure && gasInlet.Pressure.HasValue && aValue > gasInlet.Pressure.Value) {
            retValue = CreateSimpleGenericInappropriateSpecifiedValueErrorMessage(pv.VarTypeName + " of the cyclone gas outlet cannot be greater than that of the inlet.");
         }
      ...
      public override void Execute(bool propagate) {
         ...
         if (calculationType == UnitOpCalculationType.Balance) {
            ...
            if (balanceModel.IsBalanceSolved() && GasPressureDrop.HasValue
