...
namespace Prosimo.UnitOperations {
   ...
   /// <summary>
	...
	/// </summary>
	[Serializable]
   public class EvaporationAndDryingSystem : UnitOpSystem {
      ...
      public EvaporationAndDryingSystem(string name, DryingMaterial dryingMaterial, DryingGas dryingGas) : base(name) {
      ...
      public UnitOperation CreateUnitOperation(Type unitOpType) 
      {
         ...
         {
            unitOp = MakeHeater(creationType);
         }
         else if (unitOpType == typeof(Cooler)) {
            unitOp = MakeCooler(creationType);
         }
            ...
            unitOp = MakeValve(preferences.ValveStreamsType, creationType);
         }
         else if (unitOpType == typeof(FlashTank)) {
            unitOp = MakeFlashTank(creationType);
         }
         else if (unitOpType == typeof(Tee)) {
            unitOp = MakeTee(preferences.TeeStreamsType, creationType);
         }
         else if (unitOpType == typeof(Mixer)) {
            unitOp = MakeMixer(preferences.MixerStreamsType, creationType);
         }
      ...
      public PsychrometricChartModel GetPsychrometricChartModel() 
      {
         ...
         {
            DryingGasStream input = MakeDryingGasStream("Input State");
            DryingGasStream output = MakeDryingGasStream("Output State");
...
//         }
      ...
      private DryingGasStream MakeDryingGasStream(string name) {
         ...
         //return new DryingGasStream(name, m, dryingMediumType, this);
         return new DryingGasStream(name, mComponents, this);
      }
      ...
      private DryingGasStream MakeDryingGasStream() {
         ...
         //DryingGasStream gasStream = new DryingGasStream(name, dryingGas, dryingMediumType, this);
         DryingGasStream gasStream = new DryingGasStream(name, dryingGasComponents, this);
      ...
      private Heater MakeHeater(UnitOpCreationType creationType) 
      {
      ...
      private Cooler MakeCooler(UnitOpCreationType creationType) {
      ...
      private Valve MakeValve(Type streamType, UnitOpCreationType creationType) {
         ...
         ProcessStreamBase psIn;
         ProcessStreamBase psOut;
         if (creationType == UnitOpCreationType.WithInputAndOutput) {
            if (streamType == typeof(DryingGasStream)) {
               psIn = MakeDryingGasStream();
               psOut = MakeDryingGasStream();
            } 
            else {
               psIn = MakeDryingMaterialStream(MaterialStateType.Liquid);
               psOut = MakeDryingMaterialStream(MaterialStateType.Liquid);
            }

            valve.DoAttach(psIn, TwoStreamUnitOperation.INLET_INDEX);
            valve.DoAttach(psOut, TwoStreamUnitOperation.OUTLET_INDEX);
         }
         else if (creationType == UnitOpCreationType.WithInputOnly) {
            if (streamType == typeof(DryingGasStream)) {
               psIn = MakeDryingGasStream();
      ...
      private HeatExchanger MakeHeatExchanger(Type hotSideStreamType, Type coldSideStreamType, UnitOpCreationType creationType) {
         ...
         ProcessStreamBase psIn, psOut;
         if (creationType == UnitOpCreationType.WithInputAndOutput) {
            if (coldSideStreamType == typeof(DryingGasStream)) {
               psIn = MakeStream(coldSideStreamType);
               psOut = MakeStream(coldSideStreamType);
            }
            else {
               psIn = MakeDryingMaterialStream(MaterialStateType.Liquid);
               psOut = MakeDryingMaterialStream(MaterialStateType.Liquid);
            }
            exchanger.DoAttach(psIn, HeatExchanger.COLD_SIDE_INLET_INDEX);
            exchanger.DoAttach(psOut, HeatExchanger.COLD_SIDE_OUTLET_INDEX);
            
            if (hotSideStreamType == typeof(DryingGasStream)) {
               psIn = MakeStream(hotSideStreamType);
               psOut = MakeStream(hotSideStreamType);
            }
            else {
               psIn = MakeDryingMaterialStream(MaterialStateType.Liquid);
               psOut = MakeDryingMaterialStream(MaterialStateType.Liquid);
            }
            exchanger.DoAttach(psIn, HeatExchanger.HOT_SIDE_INLET_INDEX);
            exchanger.DoAttach(psOut, HeatExchanger.HOT_SIDE_OUTLET_INDEX);
         }
         else if (creationType == UnitOpCreationType.WithInputOnly) {
            if (coldSideStreamType == typeof(DryingGasStream)) {
               psIn = MakeStream(coldSideStreamType);
      ...
      private int GetUniqueIndex(ArrayList list) 
      {
         int newIndex = 0;
         Solvable solvable;
         string solvableName;
         ...
         for(int i = 0; i < list.Count; i++) {
            ...
            index = solvableName.LastIndexOf(" ");
            suffix = solvableName.Substring(index + 1);
            try {
      ...
      //persistence
      protected EvaporationAndDryingSystem (SerializationInfo info, StreamingContext context) : base (info, context) {
         //preferences = new EvaporationAndDryingSystemPreferences();
      }
      ...
      public override void SetObjectData() {
         ...
         if (persistedClassVersion == 1) {
            //this.dryingMediumType = (DryingMediumType) info.GetValue("DryingMediumType", typeof(DryingMediumType));
            ...
            //string dryMediumName = (string) info.GetValue("DryMediumName", typeof(string));
            //string moistureName = (string) info.GetValue("MoistureName", typeof(string));
            //string dryMaterialName = (string) info.GetValue("DryMaterialName", typeof(string));
            //string dryingMaterialName = info.GetValue("DryingMaterialName", typeof(string)) as string;
            //string dryingGasName = info.GetValue("DryingGasName", typeof(string)) as string;
            dryingMaterial = RecallStorableObject("DryingMaterial", typeof(DryingMaterial)) as DryingMaterial;
            dryingGas = RecallStorableObject("DryingGas", typeof(DryingGas)) as DryingGas;

            //with the dryMediumName, moistureName and dryMaterialName recalled we can 
            //initialize the dryMedium, moisture and dryMaterial substances as well as
            //moistureProperties and humidGasCalculator. We have to initialize them before
            //we recall the rest of the system so that any Solvable referencing the 
            //moistureProperties or humidGasCalculatore can be initialized at recall time.  
            //InitilizeDryingSystem(dryMediumName, moistureName, dryMaterialName); 
            //InitilizeDryingSystem(); 
            
            //this.boilingPointRiseModel = RecallStorableObject("BoilingPointRiseModel", typeof(BoilingPointRiseModel)) as BoilingPointRiseModel;
      ...
      public override void GetObjectData(SerializationInfo info, StreamingContext context) {
         ...
         //info.AddValue("DryingMediumType", this.dryingMediumType, typeof(DryingMediumType));
         info.AddValue("Preferences", this.preferences, typeof(EvaporationAndDryingSystemPreferences));
         //info.AddValue("DryMediumName", this.dryMedium.Name, typeof(string));
         //info.AddValue("MoistureName", this.moisture.Name, typeof(string));
         //info.AddValue("DryMaterialName", this.moisture.Name, typeof(string));
         //info.AddValue("BoilingPointRiseModel", boilingPointRiseModel, typeof(BoilingPointRiseModel));
         //info.AddValue("DryingMaterialName", this.dryingMaterial.Name, typeof(string));
         //info.AddValue("DryingGasName", this.dryingGas.Name, typeof(string));
