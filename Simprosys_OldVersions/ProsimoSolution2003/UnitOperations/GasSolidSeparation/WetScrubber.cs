...
namespace Prosimo.UnitOperations.GasSolidSeparation 
{
   ...
   public class WetScrubber : UnitOperation, IGasSolidSeparator 
   {
      ...
      public WetScrubber(string name, UnitOpSystem uoSys) : base(name, uoSys) 
      {
...
//      }
      ...
      private void Solve() 
      {
         ...
         //by an adiabaitc saturation process if ScrubberType is General. Moisture
         //transfers from gas stream to liquid stream by condensing if ScruberType is Condensing
         ...
         //gas state calculation
         if (scrubberType == ScrubberType.General) 
         {
         ...
         else if (scrubberType == ScrubberType.Condensing) 
         {
         ...
         if (dmsOutlet.SpecificHeatAbsDry.HasValue && inletDustMoistureFraction != Constants.NO_VALUE && gasInlet.Temperature.HasValue) 
         {
            double tempValue = gasInlet.Temperature.Value;
            double liquidCp = moistureProperties.GetSpecificHeatOfLiquid(tempValue);
            double specificHeatOfSolidPhase = (1.0 - inletDustMoistureFraction) * dmsOutlet.SpecificHeatAbsDry.Value + inletDustMoistureFraction * liquidCp;
...
//            }
         ...
         {
            Calculate(liquidRecirculationVolumeFlowRate, liquidToGasVolumeRatio.Value * gasInlet.VolumeFlowRate.Value);
         }
      }
