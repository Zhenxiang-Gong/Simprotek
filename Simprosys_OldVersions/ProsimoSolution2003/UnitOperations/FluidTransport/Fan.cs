...
//      }
      ...
      protected override ErrorMessage CheckSpecifiedValueRange(ProcessVarDouble pv, double aValue) {
         ...
         //if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.OUTLET_DIAMETER)) {
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.WIDTH))
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.HEIGHT))
         ...
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.EFFICIENCY))
         else if (pv == efficiency)
         {
            if (aValue != Constants.NO_VALUE && (aValue < 0.2 || aValue > 1.0)) {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " can not be out of the range of 0.2 to 1.");
            }
         }
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.STATIC_PRESSURE))
...
//            }
      ...
      protected Fan(SerializationInfo info, StreamingContext context) : base(info, context) {
      }
