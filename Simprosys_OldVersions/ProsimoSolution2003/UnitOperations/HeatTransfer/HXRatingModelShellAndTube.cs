...
//      }
      ...
      {
         return flowDirection == FlowDirection.Parallel && tubePassesPerShellPass.Value == 1;
      }
      ...
      internal override ErrorMessage CheckSpecifiedValueRange(ProcessVarDouble pv, double aValue) 
      {
         ...
         {
            if (aValue != Constants.NO_VALUE && (aValue < 0.00635 || aValue > 0.058)) 
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " should be in the range of 6.35 mm (1/4 inch) to 5.8 cm (2 inches)");
            }
         ...
         else if (pv == tubeInnerDiameter) 
         {
            ...
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " should not be greater than " + tubeOuterDiameter.VarTypeName);
            }
         }
         else if (pv == tubePitch)
         {
            if (tubeLayout == TubeLayout.Triangular) 
            {
               ...
               {
                  retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " for triangular tube layout must be greater than 1.25 times of " + tubeOuterDiameter.VarTypeName);
               }
            }
            else 
            {
               ...
               {
                  retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " for square tube layout must have a minimum cleaning lane of 6.35mm (1/4 inch)");
               }
            }
            if (retValue == null) 
            {
         ...
         else if (pv == bundleToShellDiametralClearance) 
         {
            ...
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " must be at least less than 5% of " + shellInnerDiameter.VarTypeName);
            }
         }
         ...
         else if (pv == baffleSpacing) 
         {
            ...
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " should be in the range of 1/5 to 1.0 the shell inner diameter with the optimum being 40-50%");
            }
         }
         else if (pv == baffleCut) 
         {
            ...
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " for Kern rating method is at 25% and can not be specified");
            }
            else if (aValue != Constants.NO_VALUE && (aValue > 0.45 || aValue < 0.15)) 
            {
               retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " should be in the range of 15-45% with the most common being 20-25%");
            }
         }
         return retValue;
      }
...
//         }
      ...
      public override double GetFtFactor (double hotIn, double hotOut, double coldIn, double coldOut) 
      {
         ...
         bool counterCurrent = flowDirection == FlowDirection.Counter ? true : false;
