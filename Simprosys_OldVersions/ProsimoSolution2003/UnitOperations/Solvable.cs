...
namespace Prosimo.UnitOperations {
   ...
   public enum SolveState {NotSolved = 0, Solved, PartiallySolved, SolveFailed};
   ...
   /// </summary>
   [Serializable]
   public abstract class Solvable : Storable, IProcessVarOwner {
      ...
      protected UnitOpSystem unitOpSystem;
      //variables to be persisted
      ...
      internal UnitOpSystem UnitOpSystem {
      ...
      internal ErrorMessage HandleException(Exception e) 
      {
         ...
         {
            errorMsg = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, e.Message); ;
         }
         ...
         {
            errorMsg = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_CALCULATED_VALUE, e.Message); ;
         }
         else if (e is CalculationFailedException) 
         {
            errorMsg = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.CALCULATION_FAILURE, e.Message); ;
         }
      ...
      {
         return new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " can not be less than or equal to 0");
      }

      internal ErrorMessage CreateLessThanZeroErrorMessage(ProcessVar pv) 
      {
         return new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " can not be less than 0");
      }

      internal ErrorMessage CreateOutOfRangeZeroToOneErrorMessage(ProcessVar pv) 
      {
         return new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, pv.VarTypeName + " can not be out of the range of 0 to 1");
      }

      #region persistence
      protected Solvable(SerializationInfo info, StreamingContext context) : base (info, context) 
      {
      }
      ...
      public override void GetObjectData(SerializationInfo info, StreamingContext context) {
         ...
         info.AddValue("UnitOpSystem", this.unitOpSystem, typeof(UnitOpSystem));
      }
