         ...
         if (pv.Type == PhysicalQuantity.Fraction) {
            if (aValue != Constants.NO_VALUE && (aValue < 0.0 || aValue > 1.0)) {
               ...
               if (totalFraction > 1.0) {
                  retValue = new ErrorMessage(ErrorType.SimpleGeneric, StringConstants.INAPPROPRIATE_SPECIFIED_VALUE, "Total stream tee fraction must not exceed 1");
               }
            }
         }
...
//      }
      ...
      private void Solve() {
         ...
         //specific heat balance for absolute dry material
         if (inlet is DryingMaterialStream) {
            numOfKnown = 0;
            DryingStream dsStream;
            for (int i = 0; i < InOutletStreams.Count; i++) {
               dsStream = InOutletStreams[i] as DryingMaterialStream;
               if (dsStream.SpecificHeatAbsDry.HasValue) {
                  numOfKnown++;
                  j = i;
               }
            }
            if (numOfKnown == 1) {
               ...
               temp = dsStream.SpecificHeatAbsDry.Value;
               for (int i = 0; i < InOutletStreams.Count; i++) {
                  if (i != j) {
                     dsStream = InOutletStreams[i] as DryingMaterialStream;
                     Calculate(dsStream.SpecificHeatAbsDry, temp);
                  }
               }
            }
         }
