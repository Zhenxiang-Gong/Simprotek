...
namespace Prosimo.UnitOperations
{
   ...
   public enum FlowDirection {Counter = 0, Parallel, Cross};
...
//         }
      ...
      public ErrorMessage SpecifyCalculationType(UnitOpCalculationType aValue) 
      {
         ErrorMessage retMsg = null;
      ...
      private void EnableBalanceOrRatingModel (UnitOpCalculationType calcType) 
      {
      ...
      protected void BalanceAdiabaticProcess(ProcessStreamBase inlet, ProcessStreamBase outlet) 
      {
         if (inlet.Pressure.HasValue && outlet.Pressure.HasValue) 
         {
            ...
            else if (outlet.Temperature.HasValue) 
            {
               k = inlet.GetSpecificHeatRatio(inlet.Temperature.Value);
               tempValue = Math.Pow(pRatio, (k-1.0)/k);
      ...
      protected void UpdateStreams() 
      {
         ...
         foreach (ProcessStreamBase ps in streamList) 
         {
            ...
            {
               ps.OnSolveComplete(ps.SolveState);
            }
         }
      }
      ...
      protected UnitOperation(SerializationInfo info, StreamingContext context) : base(info, context) {
      }
