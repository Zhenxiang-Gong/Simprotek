...
//      }
      ...
      protected Dryer(string name, UnitOpSystem uoSys) : base(name, uoSys) 
      {
      ...
      public override bool CanAttachStream(ProcessStreamBase ps, int streamIndex) {
         if (((streamIndex == 0 || streamIndex == 2) && ps.DownStreamOwner != null) 
            || ((streamIndex == 1 || streamIndex == 3) && ps.UpStreamOwner != null)) {
            return false;
         }

         bool canAttach = false;
         if (ps is DryingGasStream && (streamIndex == 0 || streamIndex == 1)) {
            if (streamIndex == GAS_INLET_INDEX && gasInlet == null) {
               canAttach = true;
            }
            else if (streamIndex == GAS_OUTLET_INDEX && gasOutlet == null) {
         ...
         else if (ps is DryingMaterialStream && (streamIndex == 2 || streamIndex == 3)) {
      ...
      internal override bool DoAttach(ProcessStreamBase ps, int streamIndex) {
         ...
         else if (streamIndex == 2 || streamIndex == 3) {
...
//      
      protected override ErrorMessage CheckSpecifiedValueRange(ProcessVarDouble pv, double aValue) 
      {
         ...
         //if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.FRACTION_OF_MATERIAL_LOST_TO_GAS_OUTLET)) {
         if (pv == fractionOfMaterialLostToGasOutlet) {
            if (aValue != Constants.NO_VALUE && (aValue < 0.0 || aValue > 1.0)) 
            {
               retValue = CreateOutOfRangeZeroToOneErrorMessage(pv);;
            }
         }
         //else if (pv.VarTypeName == StringConstants.GetTypeName(StringConstants.HEAT_LOSS_BY_TRANSPORT_DEVICE)) {
      ...
      //   ModifyMassflowAndCalculateComponents();
      ...
      public override void Execute(bool propagate) {
         ...
         BalanceDryingMaterialStreamSpecificHeat(materialInlet, materialOutlet);
      ...
      private void Solve() {
         ...
         double cs = materialInlet.SpecificHeatAbsDry.Value;
         ...
         ///case 8
            ...
            if (ws != Constants.NO_VALUE && x1 != Constants.NO_VALUE && x2 != Constants.NO_VALUE) {
               ...
               } while (Math.Abs(tg2 - tg2_old) > 1.0e-6 && counter <= 200);
               ...
               do {
                  ...
                  cm = cs + cw * x2;
                  ModifyDelta(tg1, tg2_old, ref tg2, ref negativeLastTime, ref delta, ref totalDelta);
               } while (Math.Abs(cm - cm_old) > 1.0e-6 && Math.Abs(tg2 - tg2_old) > 1.0e-6 && counter <= 200);
               ...
               do {
                  ...
                  tg2 = CalculateTg2(wg, cg1, tg1, w, r0, cv, cw, tm1, q2, q3-q4-q5);
                  ModifyDelta(tg1, tg2_old, ref tg2, ref negativeLastTime, ref delta, ref totalDelta);
               } while (Math.Abs(tg2 - tg2_old) > 1.0e-6 && counter <= 50);
      ...
      protected Dryer(SerializationInfo info, StreamingContext context) : base(info, context) {
      }
