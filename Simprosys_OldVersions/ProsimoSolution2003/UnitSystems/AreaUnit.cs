...
namespace Prosimo.UnitSystems
{
   ...
   public class AreaUnit {
      ...
      public static double ConvertToSIValue(AreaUnitType unitType, double toBeConvertedValue) {
         double convertionCoeff = (double) coeffTable[unitType];
         return convertionCoeff * toBeConvertedValue;
      }
      
      public static double ConvertFromSIValue(AreaUnitType unitType, double sqMeterValue) {
         double convertionCoeff = (double) coeffTable[unitType];
         return sqMeterValue/convertionCoeff;
      }
      
      public static string GetUnitAsString(AreaUnitType unitType) {
         return unitStringTable[unitType] as String;
      }
      
      public static AreaUnitType GetUnitAsEnum(string unitString) {
         IDictionaryEnumerator myEnumerator = unitStringTable.GetEnumerator();
         String name;
         AreaUnitType type = AreaUnitType.SquareMeter;
         while (myEnumerator.MoveNext()) {
            name = myEnumerator.Value as String;
            if (name.Equals(unitString)) {
               type = (AreaUnitType) myEnumerator.Key;
               break;
            }
         }
         return type;
