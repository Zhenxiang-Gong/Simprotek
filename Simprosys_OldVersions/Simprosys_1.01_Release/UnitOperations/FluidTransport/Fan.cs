...
namespace Prosimo.UnitOperations.FluidTransport {
   ...
   public class Fan : TwoStreamUnitOperation {
      ...
      public Fan(string name, UnitOperationSystem uoSys)
         ...
         includeOutletVelocityEffect = false;
      ...
      public override void Execute(bool propagate) {
         ...
         CalculateAdiabaticCompressionProcess();
      ...
      private void CalculateAdiabaticCompressionProcess() {
         if (inlet.Pressure.HasValue && outlet.Pressure.HasValue) {
            if (inlet is DryingGasStream) {
               if (!(inlet as DryingGasStream).Humidity.HasValue) {
                  return;
               }
            }
            double t1;
