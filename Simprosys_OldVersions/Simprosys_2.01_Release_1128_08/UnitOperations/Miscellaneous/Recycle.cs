      ...
      public override void Execute(bool propagate) {
         ...
         if (inlet is DryingStream) {
            ...
            do {
               ...
               //need to catch any exception if calculation fails to clean up the specified values
               //for the outletVarList
               try {
                  dsOutlet.HasBeenModified(true);
               }
               catch {
                  for (int i = 0; i < outletVarList.Count; i++) {
                     outletVarList[i].Value = Constants.NO_VALUE;
                  }
            ...
            else if (solveState == SolveState.NotSolved) {
            ...
            dsOutlet.OnSolveComplete();//has to call this so that the calculated state is updated on the UI
         }
