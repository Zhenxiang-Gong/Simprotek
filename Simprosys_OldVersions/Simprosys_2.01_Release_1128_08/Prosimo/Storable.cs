...
namespace Prosimo {
   ...
   public abstract class Storable : ISerializable {
      ...
      public static Storable RecallStorableObject(SerializationInfo info, StreamingContext context, string objectName, Type objectType) {
         Storable storable = info.GetValue(objectName, objectType) as Storable;
         if (storable != null) {
            storable.SetObjectData();
         }
      ...
      protected IList<T> RecallGenericListObject<T>(string listName) {
         IList list = info.GetValue(listName, typeof(IList)) as IList;
         if (list != null) {
            foreach (Storable obj in list) {
               obj.SetObjectData();
            }
