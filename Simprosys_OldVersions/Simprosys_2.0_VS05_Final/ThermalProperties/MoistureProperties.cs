...
namespace Prosimo.ThermalProperties {
   ...
   public class MoistureProperties {
      ...
      public MoistureProperties(Substance moisture) {
         ...
         criticalTemperature = moisture.CriticalPropsAndAccentricFactor.CriticalTemperature;
      ...
      public double GetDensity(double pressure, double temperature) {
         ...
         double density = thermalPropCalculator.CalculateLiquidDensity(pressure, temperature, moisture);
         return density;
      }
      ...
      public double GetSpecificHeatOfLiquid(double temperature) {
         ...
         double cp = thermalPropCalculator.CalculateLiquidHeatCapacity(temperature, moisture);
      ...
      public double GetMeanSpecificHeatOfLiquid(double t1, double t2) {
         ...
         //double cp = 2.7637e5 - 2.0901e3 * temperature + 8.125 * temperature * temperature
         //            -1.4116e-2 * Math.Pow(temperature, 3) + 9.3701e-6 * Math.Pow(temperature, 4);
         double cp = thermalPropCalculator.CalculateLiquidMeanHeatCapacity(t1, t2, moisture);
         //return cp / molarWeight;
         return cp;
      }
      ...
      public double GetMeanSpecificHeatOfLiquid(double p, double t1, double t2) {
         ...
         //}
         //else {
         //   cp = thermalPropCalculator.CalculateMeanLiquidHeatCapacity(t1, t2, moisture);
         //   cp = cp / molarWeight;
         //}
         double cp = thermalPropCalculator.CalculateLiquidMeanHeatCapacity(p, t1, t2, moisture);
         return cp;
      }
