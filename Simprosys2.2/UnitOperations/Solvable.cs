...
namespace Prosimo.UnitOperations {
   ...
   public abstract class Solvable : Storable, IProcessVarOwner, IComparable {
      ...
      public ErrorMessage Specify(Hashtable procVarAndValueTable) {
         ...
         foreach (ProcessVar pv in procVarAndValueTable.Keys) {
            thePV = pv;
         ...
         try {
            ...
            //while (iter.MoveNext()) {
            //   pv = (ProcessVar)iter.Current;
            foreach (ProcessVar pv in oldValueTable.Keys) {
               if (pv is ProcessVarDouble) {
                  pvDouble = pv as ProcessVarDouble;
         ...
         OnProcessVarValueCommitted(thePV);
      ...
      public override void SetObjectData() {
         base.SetObjectData();
         int persistedClassVersion = info.GetInt32("ClassPersistenceVersionSolvable");
