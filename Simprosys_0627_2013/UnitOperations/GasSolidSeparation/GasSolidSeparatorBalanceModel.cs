...
namespace Prosimo.UnitOperations.GasSolidSeparation {
   ...
   internal class GasSolidSeparatorBalanceModel : Storable{
      ...
      internal double CalculateParticleLoading(ProcessStreamBase psb) {
         ...
         SolidPhase solidPhase = dgc.SolidPhase;
         double massFlow = stream.MassFlowRate.Value;
         double volumeFlow = stream.VolumeFlowRate.Value;
         double loading = Constants.NO_VALUE;
         if (solidPhase != null && massFlow != Constants.NO_VALUE && volumeFlow != Constants.NO_VALUE) {
            double massFlowOfParticle = solidPhase.MassFraction * massFlow;
            loading = massFlowOfParticle/volumeFlow;
         }
         return loading;
      }

      internal bool IsBalanceSolved() {
         bool isBalanceSolved = false;
         if (collectionEfficiency.HasValue && inletParticleLoading.HasValue && outletParticleLoading.HasValue &&
            particleCollectionRate.HasValue && massFlowRateOfParticleLostToGasOutlet.HasValue) {
            isBalanceSolved = true;
         }
         return isBalanceSolved;
      }
