...
namespace Prosimo.UnitOperations.Drying {
   public enum HCType {GasState, IsenthalpicProcess};
   ...
   public class PsychrometricChartModel : UnitOperation {
      ...
      private static IDictionary<string, double> humidityScaleTable = new Dictionary<string, double>();
      ...
      public string SystemSuffixName {
         get { return ((EvaporationAndDryingSystem)(unitOpSystem)).SystemSuffixName; }
      }

      public event HumidityChartChangedEventHandler HumidityChartChanged;
      public event HCTypeChangedEventHandler HCTypeChanged;

      static PsychrometricChartModel() {
         humidityScaleTable.Add(Substance.WATER, 0.3);
         humidityScaleTable.Add(Substance.CARBON_TETRACHLORIDE, 2.0);
         humidityScaleTable.Add(Substance.BENZENE, 2.0);
         humidityScaleTable.Add(Substance.TOLUENE, 0.4);
         humidityScaleTable.Add(Substance.ACETONE, 4.0);
         humidityScaleTable.Add(Substance.METHANOL, 1.0);
         humidityScaleTable.Add(Substance.ETHANOL, 1.0);
         humidityScaleTable.Add(Substance.N_PROPANOL, 0.3);
         humidityScaleTable.Add(Substance.ISO_PROPANOL, 0.5);
         humidityScaleTable.Add(Substance.N_BUTANOL, 0.3);
         humidityScaleTable.Add(Substance.ISO_BUTANOL, 0.3);
         humidityScaleTable.Add(Substance.ACETIC_ACID, 0.5);
      }
      ...
      public ErrorMessage SetCurrentState(PointF pt) {
         ...
         if (msg == null) {
            ...
            retMsg = CreateSimpleGenericInappropriateSpecifiedValueErrorMessage(msg);
         }
      ...
      private HumidGasCalculator GetHumidGasCalculator() {
         return GetHumidGasCalculator(inputStream);
      }
      ...
      private CurveFamilyF GenerateAdiabaticSaturationFamily() {
         ...
         for (int i = 0; i < numOfCurves; i++) {
            tsat = xVar.Min + i * 5.0;
            ysat = humidGasCalculator.GetHumidityFromDewPointAndPressure(tsat, p);
            ih = (cg + cv * ysat) * (tsat - 273.15) + r0 * ysat;
            maxTemp = ih / cg + 273.15;
...
//
      public override void Execute(bool propagate) {
         ...
         if (hcType == HCType.IsenthalpicProcess) 
         {
            Solve();
