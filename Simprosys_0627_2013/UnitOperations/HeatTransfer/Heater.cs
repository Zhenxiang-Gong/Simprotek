...
namespace Prosimo.UnitOperations.HeatTransfer {
   /// <summary>
   /// Summary description for Heater.
   /// </summary>
   [Serializable]
   public class Heater : TwoStreamUnitOperation {
      ...
      internal override ErrorMessage CheckSpecifiedValueInContext(ProcessVarDouble pv, double aValue) {
         ...
         if (pv == inlet.Pressure && outlet.Pressure.IsSpecifiedAndHasValue && aValue < outlet.Pressure.Value) {
            retValue = CreateSimpleGenericInappropriateSpecifiedValueErrorMessage(pv.VarTypeName + " of the heater inlet must be greater than that of the outlet.");
         }
         else if (pv == outlet.Pressure && inlet.Pressure.IsSpecifiedAndHasValue && aValue > inlet.Pressure.Value) {
            retValue = CreateSimpleGenericInappropriateSpecifiedValueErrorMessage(pv.VarTypeName + " of the heater outlet cannot be greater than that of the inlet.");
         }
      ...
      private void Solve() {
         ...
         //double t1 = inlet.SpecificEnthalpy.Value;
         //double t2 = outlet.SpecificEnthalpy.Value;
      ...
      public override void SetObjectData() {
         base.SetObjectData();
         int persistedClassVersion = (int)info.GetValue("ClassPersistenceVersionHeater", typeof(int));
      ...
      public override void GetObjectData(SerializationInfo info, StreamingContext context) {
         ...
         info.AddValue("ClassPersistenceVersionHeater", Heater.CLASS_PERSISTENCE_VERSION, typeof(int));
