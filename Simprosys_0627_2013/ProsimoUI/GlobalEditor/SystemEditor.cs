...
namespace ProsimoUI.GlobalEditor {
   ...
   public class SystemEditor : System.Windows.Forms.Form {
      ...
      private FuelStreamsEditor fuelStreamsEditor;
      ...
      private BurnersEditor burnersEditor;
      ...
      public int SplitterPosition {
         get { return splitterStreams_UnitOps.SplitPosition; }
         set { splitterStreams_UnitOps.SplitPosition = value; }
      }
      ...
      /// <summary>
      /// Closing should not dispose the form since user may open it at a later time. 
      /// If it is closed and everything is disposed the adjusted size of the form would be 
      /// lost. Please be careful when it is closed forever, the Dispose method should be called
      /// explicitly
      /// </summary>
      ...
      private void PrepareToPrint() {
         ...
         list = this.flowsheet.StreamManager.GetShowableInEditorStreamControls<DetailedFuelStream>();
         this.AddListToStringBuilder(list, sb);

         list = this.flowsheet.UnitOpManager.GetShowableInEditorUnitOpControls<AirFilter>();
         this.AddListToStringBuilder(list, sb);

         list = this.flowsheet.UnitOpManager.GetShowableInEditorUnitOpControls<Burner>();
         this.AddListToStringBuilder(list, sb);
      ...
      public void UpdateStreamsUI() {
         ...
         if (this.flowsheet.StreamManager.HasShowableInEditorStreamControls<DetailedFuelStream>())
         {
            if (this.fuelStreamsEditor == null)
               this.fuelStreamsEditor = new FuelStreamsEditor(this.flowsheet);
            else
               this.fuelStreamsEditor.InitializeUI();
            this.fuelStreamsEditor.Location = new Point(0, y);
            this.panelStreams.Controls.Add(this.fuelStreamsEditor);
            y += this.fuelStreamsEditor.Height;
         }
         else
         {
            if (this.fuelStreamsEditor != null)
               this.fuelStreamsEditor.Dispose();
            this.fuelStreamsEditor = null;
         }
      ...
      public void UpdateUnitOpsUI() {
         ...
         if (this.flowsheet.UnitOpManager.HasShowableInEditorUnitOpControls<Burner>())
         {
            if (this.burnersEditor == null)
               this.burnersEditor = new BurnersEditor(this.flowsheet);
            else
               this.burnersEditor.InitializeUI();
            this.burnersEditor.Location = new Point(0, y);
            this.panelUnitOps.Controls.Add(this.burnersEditor);
            y += this.burnersEditor.Height;
         }
         else
         {
            if (this.burnersEditor != null)
               this.burnersEditor.Dispose();
            this.burnersEditor = null;
         }
