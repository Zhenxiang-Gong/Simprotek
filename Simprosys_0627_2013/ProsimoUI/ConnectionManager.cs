...
namespace ProsimoUI {
   ...
   public class ConnectionManager : Storable {
      ...
      public void UpdateConnections(ElectrostaticPrecipitatorControl unitOpCtrl)
      {
         string name = unitOpCtrl.UnitOperation.Name;

         PointOrientation inOrientation = ElectrostaticPrecipitatorControl.INLET_ORIENTATION;

         int inIdx = ElectrostaticPrecipitator.INLET_INDEX;
         Point inPoint = unitOpCtrl.GetStreamInConnectionPoint();
         ConnectionPoint inCp = new ConnectionPoint(inIdx, name, inPoint, inOrientation);

         PointOrientation outOrientation = ElectrostaticPrecipitatorControl.OUTLET_ORIENTATION;

         int outIdx = ElectrostaticPrecipitator.OUTLET_INDEX;
         Point outPoint = unitOpCtrl.GetStreamOutConnectionPoint();
         ConnectionPoint outCp = new ConnectionPoint(outIdx, name, outPoint, outOrientation);

         PointOrientation particleOutOrientation = ElectrostaticPrecipitatorControl.PARTICLE_OUTLET_ORIENTATION;
         int particleOutIdx = BagFilter.PARTICLE_OUTLET_INDEX;
         Point particleOutPoint = unitOpCtrl.GetParticleOutConnectionPoint();
