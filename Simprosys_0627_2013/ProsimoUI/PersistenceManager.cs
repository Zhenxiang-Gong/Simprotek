   ...
   public class PersistenceManager {
      ...
      public Flowsheet UnpersistFlowsheet(string fileName, ApplicationPreferences appPrefs) {
         ...
         try {
            ...
            //ArrayList persistedItems = (ArrayList)Recall(fileName);
            ...
            if(flowsheet != null)
            {
               ...
               flowsheet.RecallInitialization(flowsheet.Version.Major, flowsheet.Version.Minor);
            }
         }
         catch (Exception e) {
      ...
      public void PersistFossilFuelCatalog(string fileName)
      {
         try
         {
            IList customMaterials = FossilFuelCatalog.Instance.GetFossilFuelList(true);
            PersistFile(fileName, customMaterials);
         }
         catch(Exception e)
         {
            string message = e.ToString();
            MessageBox.Show(message, "Error Saving Fuel Catalog", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
         }
      }

      public bool UnpersistFossilFuelCatalog(string fileName)
      {
         bool ok = true;
         try
         {
            ...
            if(File.Exists(fileName))
            {
               IList customFuelList = (IList)UnpersistFile(fileName);

               foreach(FossilFuel ff in customFuelList)
               {
                  ff.SetObjectData();
...
//private int GetFlowsheetVersion() {
...
//         string[] subStrs = line.Trim().Split(new string[2] {"<Major>", "</Major>"}, StringSplitOptions.RemoveEmptyEntries);
//         major = int.Parse(subStrs[0]);
//      }
//      if (line.Contains("</Minor>")) {
//         string[] subStrs = line.Trim().Split(new string[2] { "<Minor>", "</Minor>" }, StringSplitOptions.RemoveEmptyEntries);
//         minor = int.Parse(subStrs[0]);
//      }
//   }
//   return major
//}
