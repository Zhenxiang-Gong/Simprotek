...
namespace Prosimo.ThermalProperties {
   ...
   public class ThermalPropCalculator {
      ...
      private IList perrysGasCpCorrelationList;
      ...
      private void LoadPerrysThermalPropCorrelations(IFormatter serializer) {
         ...
         perrysGasCpCorrelationList = thermalPropCorrelationList;
      ...
      public IList GetPerrySubstanceNameList()
      {
         IList list = new ArrayList();
         foreach (PerrysGasCpCorrelation correlation in perrysGasCpCorrelationList)
         {
            list.Add(correlation.SubstanceName);
         }
      ...
      {
         double cp = 0;
         if(s is CompositeSubstance)
         {
            CompositeSubstance compSub = s as CompositeSubstance;
            foreach(MaterialComponent component in compSub.Components)
            {
               double sbscCp = GetGasHeatCapacity(t, component.Substance);
               cp += sbscCp * component.MassFraction.Value;
            }
         }
         else
         {
            cp = GetGasHeatCapacity(t, s);
         }
      ...
      /// <param name="s">The substance to be calculated</param>
      /// <returns>Calculated heat capacity value (unit is J/kg.K)</returns>
      private double GetGasHeatCapacity(double t, Substance s) {
      ...
      public double CalculateGasMeanHeatCapacity(double t1, double t2, Substance s) {
         ...
         if(s is CompositeSubstance)
         {
            ...
            foreach(MaterialComponent component in compSub.Components)
            {
               double sbscCp = GetGasMeanHeatCapacity(t1, t2, component.Substance);
               cp += sbscCp * component.MassFraction.Value;
            }
         }
         else
         {
            cp = GetGasMeanHeatCapacity(t1, t2, s);
         }

         return cp;
      }

      //calculated value unit is J/kg.K
      private double GetGasMeanHeatCapacity(double t1, double t2, Substance s) {
         double cp = Constants.NO_VALUE;
         PerrysGasCpCorrelation correlationPerrys = GetPerrysGasCpCorrelation(s);
         if (correlationPerrys != null) {
      ...
      public double CalculateGasThermalConductivity(double t, Substance s) {
         ...
         if(s is CompositeSubstance)
         {
            ...
            foreach(MaterialComponent component in compSub.Components)
            {
               double sbscK = GetGasThermalConductivity(t, component.Substance);
               k += sbscK * component.MoleFraction.Value;
            }
         }
         else
         {
            k = GetGasThermalConductivity(t, s);
         }
         return k;
      }

      //calculated value unit is w/m.K, t unit is K
      private double GetGasThermalConductivity(double t, Substance s)
      {
      ...
      //calculated value unit is J/kg, t unit is K
      public double CalculateEnthalpyOfFormationMassBasis(double t, Substance s)
      {
         return CalculateEnthalpyOfFormation(t, s) / s.MolarWeight;
      }
